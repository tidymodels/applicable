...
To analyze the Jaccard metric, we have the following methods

* `apd_similarity`: analyzes samples in terms of similarity scores. For a training set of _n_ samples, a new sample is compared to each, resulting in _n_ similarity scores. These can be summarized into the median similarity.

* `autoplot`: shows the cumulative probability versus the unique similarity values in the training set.

* `score`: Score new samples using similarity methods. In particular, it calculates the similarity scores and if `add_percentile = TRUE`, it also estimates the percentile of the similarity scores.
