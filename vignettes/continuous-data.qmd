...
vignette: >
  %\VignetteIndexEntry{Applicability domain methods for continuous data}
...
More details can be found in [De Cock (2011, Journal of Statistics Education)](https://jse.amstat.org/v19n3/decock.pdf).
...
#| fig-align: center
#| fig-width: 6
#| fig-alt: "Faceted empirical cumulative distribution chart for PCA-derived metrics, with absolute metric values on the x-axis and percentile on the y-axis."
library(ggplot2)
autoplot(ames_pca)
```
...
#| echo: false
#| results: hold
#| fig-alt: "Two PCA distribution plots: one for components PC1 through PC5, and one for distance values on a log-scaled x-axis, each showing percentile on the y-axis."
...
```{r}
#| echo: false
#| fig-alt: "Histogram of first principal-component training scores with red vertical lines marking the three new-sample PC01 values."
...
they were new homes. The also tend to have fairly large gross living areas:

```{r}
#| echo: false
#| fig-alt: "Histogram of gross living area in the training data with red vertical lines showing the gross living area for the three new properties."
