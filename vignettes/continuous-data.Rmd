...
training_recipe <-
  recipe(Sale_Price ~ ., data = training_data) %>%
...
Suppose that we had a new, unknown sample (as a $p \times 1$ data vector $\mathbf{u}$). The hat value for this sample would be 

$$\mathbf{u}'(X'X)^{-1}\mathbf{u}$$.
