...
To explain these methods, let us take a look at an example.
...
Setting `threshold = 0.43` or 43%, we now need only 16 principal components:

```{r}
ames_ad <- apd_pca(training_recipe, training_data, threshold = 0.43)
ames_ad
```
...
```{r, echo = FALSE}
par(mfrow=c(1,2))
autoplot(ames_ad, matches("PC0[1-5]"))
